...
namespace VRTK
{
    ...
    public delegate void CustomObjectInteractEventHandler(object sender, GameObject target);
    ...
    public class VRTK_InteractTouch : MonoBehaviour
    {
        ...
        // Jesse magic
        public event CustomObjectInteractEventHandler ControllerTouchSwapZone;
        public event CustomObjectInteractEventHandler ControllerUntouchSwapZone;
        ...
        {
            // XXX: Jesse Magic
            if (collider.name == "SwapControllerZone" && touchedObject != null && touchedObject.GetComponent<VRTK_InteractableObject>() != null) {
                VRTK_InteractableObject touchedObjectScript = touchedObject.GetComponent<VRTK_InteractableObject>();
                if (touchedObjectScript.secondaryGrabActionScript.isSwappable) {
                    touchedObjectScript.secondaryGrabActionScript.isSwappable = false;
                    //Debug.Log("Turnin that shit off!");

                    if (collider.transform.Find("Preview")) {
                        collider.transform.Find("Preview").gameObject.SetActive(false);
                    }
                    
                    // Emit untouch event of swap zone
                    if (ControllerUntouchSwapZone != null) {
                        ControllerUntouchSwapZone(this, collider.gameObject);
                    }
                }
            }
