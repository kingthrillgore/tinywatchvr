...
namespace VRTK
{
    ...
    public class VRTK_SlideObjectControlAction : VRTK_BaseObjectControlAction
    {
        ...
        // XXX: Jesse Magic
        public delegate void VRTK_MoveEvent();
        public static VRTK_MoveEvent VRTK_MoveStarted;
        public static VRTK_MoveEvent VRTK_MoveEnded;
        ...
        protected virtual void Move(GameObject controlledGameObject, Transform directionDevice, Vector3 axisDirection)
        {
            if (directionDevice && directionDevice.gameObject.activeInHierarchy && controlledGameObject.activeInHierarchy)
            {
                ...
                // XXX: Jesse Magic
                bool onNavMesh = true;
                Transform colliderToCheck = controlledGameObject.transform;
                if (collisionOrigin) {
                    colliderToCheck = collisionOrigin;

                    Vector3 finalPositionForOrigin = colliderToCheck.position + updatedPosition;
                    onNavMesh = DestinationOnNavMesh(colliderToCheck.position, finalPositionForOrigin);
                }

                // XXX: Jesse Magic
                if (CanMove(bodyPhysics, controlledGameObject.transform.position, finalPosition) && onNavMesh) {
                    // XXX: Jesse Magic
        ...
        // XXX: Jesse Magic
        bool DestinationOnNavMesh(Vector3 origin, Vector3 destination) {
            NavMeshHit hitData;
            bool hit = NavMesh.Raycast(origin, destination, out hitData, NavMesh.AllAreas);
